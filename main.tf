...
resource "aws_s3_bucket" "static_web" {
  bucket        = "joannews3cf" #Name of your bucket
  force_destroy = true
}
...
resource "aws_cloudfront_distribution" "s3_distribution" {
  ...
  enabled             = true
  comment             = "joannews3CF's static website using Cloudfront" #Description
  default_root_object = "index.html"

  aliases = ["joannews3cf.sctp-sandbox.com"] #Cloudfront alternate domain name (Same as Route53 record)
...
resource "aws_cloudfront_origin_access_control" "oac" {
  name                              = "joannews3cf-cloudfront-oac"    #e.g. <yourname>-cloudfront-oac
...
resource "aws_route53_record" "www" {
  ...
  name    = "joannews3cf"  #Your domain prefix. <this-value>.sctp-sandbox.com
  type    = "A"
